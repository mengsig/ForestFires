...
# convert to cubes
slope_cube = normalize(slope, datatype="slp")
aspect_cube = normalize(aspect, datatype="asp")
dem_cube = normalize(dem, datatype="dem")
cc_cube = normalize(cc, datatype="cc")
cbd_cube = normalize(cbd, datatype="cbd")
cbh_cube = normalize(cbh, datatype="cbh")
ch_cube = normalize(ch, datatype="ch")
fuel_model_cube = normalize(fuel_model, datatype="fbfm")
...
print(
    f"[SIMULATE-{centrality}]: Total Runtime: " + str(simulation_runtime) + " seconds"
)
print(
    f"[SIMULATE-{centrality}]: Runtime Per Burned Cell: "
    + str(runtime_per_burned_cell)
    + " ms/cell"
)
...
# ----------------- Saving statistics -----------------
vmin, vmax = (
    output_matrices["fireline_intensity"].min() + 1,
    output_matrices["fireline_intensity"].max(),
)
...
with open(out_path, "w") as fout, redirect_stdout(fout):
    ...
    pprint(
        get_array_stats(output_matrices["phi"], use_burn_scar_mask=False),
        sort_dicts=False,
    )
